...
def routes(selectedCity="Sedona Area"):
    ...
    routes_response = []
    for ea in result:
        routes_response.append({"route": ea[0], "lat": ea[1], "lon": ea[2]})
...
def dispensaries(disp_lat=34.1403, disp_lon=-118.3838):
    ...
    query = db.select([table.c.name, table.c.lat, table.c.lon]).where(table.columns.lat < upper_lat).where(table.columns.lat > lower_lat).where(table.columns.lon < upper_lon).where(table.columns.lon > lower_lon)
    result = connection.execute(query).fetchall()
    # print(type(dict(result[0])))
    # print(result)
    routes_response = []
    for ea in result:
        routes_response.append({"dispensary": ea[0], "lat": ea[1], "lon": ea[2]})
